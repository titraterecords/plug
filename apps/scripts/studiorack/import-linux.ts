// Imports Linux plugins from the Open Audio Stack registry.
// Downloads each archive, scans for .vst3/.clap/.lv2 artifacts,
// and merges into our registry.json with verified artifact names.
// Skips individual versions that already have linux data in the registry.
//
// Usage: pnpm import:linux
...
async function main(): Promise<void> {
  ...
  console.log(`  Enriched: ${enriched.length} existing plugins with linux data`);
  if (added.length > 0) {
